...
describe('adnuntiusBidAdapter', function() {
  ...
  const ENDPOINT_URL_DEALS = `${ENDPOINT_URL_BASE}&userId=${usi}&ds=2`;
  ...
  describe('buildRequests', function() {
    it('Test requests', function() {
      ...
      expect(request[0].data).to.equal('{"adUnits":[{"auId":"000000000008b6bc","targetId":"adn-000000000008b6bc","dimensions":[[640,480],[600,400]]},{"auId":"0000000000000551","targetId":"adn-0000000000000551","dimensions":[[1640,1480],[1600,1400]]}],"metaData":{"valid":"also-valid","usi":"' + usi + '"}}');
    });
    ...
    it('Test requests with no local storage', function() {
      ...
      expect(request[0]).to.have.property('data');
      expect(request[0].data).to.equal('{"adUnits":[{"auId":"000000000008b6bc","targetId":"adn-000000000008b6bc","dimensions":[[640,480],[600,400]]},{"auId":"0000000000000551","targetId":"adn-0000000000000551","dimensions":[[1640,1480],[1600,1400]]}]}');
    ...
    it('Test request changes for voided au ids', function() {
      storage.setDataInLocalStorage('adn.meta', JSON.stringify([{key: 'voidAuIds', value: [{auId: '0000000000000551', exp: misc.getUnixTimestamp(1)}]}]));
      const request = spec.buildRequests(bidderRequests, {});
      expect(request.length).to.equal(1);
      expect(request[0]).to.have.property('bid');
      const bid = request[0].bid[0]
      ...
      expect(request[0].url).to.equal(ENDPOINT_URL_BASE);
      expect(request[0]).to.have.property('data');
      expect(request[0].data).to.equal('{"adUnits":[{"auId":"000000000008b6bc","targetId":"adn-000000000008b6bc","dimensions":[[640,480],[600,400]]}]}');
    });
  ...
  describe('request deals', function() {
    it('Should request deals.', function() {
      config.setBidderConfig({
        bidders: ['adnuntius'],
        config: {
          maxDeals: 2
      ...
      expect(request.length).to.equal(1);
      expect(request[0]).to.have.property('url')
      expect(request[0].url).to.equal(ENDPOINT_URL_DEALS);
    });
  });
